...
export class SolanaAgentKit {
  ...
  constructor(
    ...
    this.wallet = new PhantomWallet(publicKey);
  ...
  async get_token_balance(): Promise<{
    sol: number;
    tokens: Array<{
      tokenAddress: string;
      name: string;
    ...
    return get_token_balance(this);
  }

  async fetchPythPrice(symbol: string): Promise<number> {
    return fetchPythPrice(this, symbol);
  }
}
...
class PhantomWallet {
  ...
  get publicKey(): PublicKey {
    return new PublicKey(this.publicKeyStr);
  }

  async signTransaction<T extends Transaction | VersionedTransaction>(tx: T): Promise<T> {
    // This will be handled by the frontend
    throw new Error("Transaction signing should be handled by the frontend");
  }
